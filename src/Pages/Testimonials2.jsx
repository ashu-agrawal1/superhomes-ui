...
import { addReview, fetchAllReviews, updateReviewById } from "../Redux/TestimonialSlice"
...
const ReviewCard = styled.div`
  ...
  width: 300px;
  flex-shrink: 0;
  // Remove the margin-right from here
...
const Testimonials = () => {
  ...
  const handleUpdateReview = (updatedReview) => {
    ...
    const updatedData = {
      ...updatedReview,
      user_name: localReviews.find((review) => review.id === updatedReview.id)?.user_name,
    }
  ...
  const StarRating = ({ rating, onRatingChange }) => (
    <StarRatingContainer>
      {[...Array(5)].map((_, index) => (
        <StarButton key={index} type="button" isActive={index < rating} onClick={() => onRatingChange(index + 1)}>
    ...
    <Modal>
      <ModalContent>
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setIsEditReviewModalOpen(false)} className="text-gray-500 hover:text-gray-700">
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <Button
          onClick={() => handleUpdateReview({ id: editingReview.id, rating, message: reviewTextRef.current.value })}
  ...
  const renderRows = () => {
    ...
    return (
      <>
        <CardsRow>
          <InnerRowLeft ref={rowLeftRef}>{renderCards(topRowCards)}</InnerRowLeft>
        </CardsRow>
        <CardsRow>
          <InnerRowRight ref={rowRightRef}>{renderCards(bottomRowCards)}</InnerRowRight>
        </CardsRow>
      </>
    )
  }
    ...
    <Container>
      <Header>
        <div className="flex items-center justify-center gap-4 mb-2">
          <span className="text-5xl font-semibold text-gray-800">4.88</span>
        </div>
        <h2 className="text-2xl font-semibold mb-2 text-gray-800">Guest favourite</h2>
      ...
      <div className="flex gap-4 mt-6">
        <Button onClick={() => setIsModalOpen(true)}>Show all {reviews?.length || 0} reviews</Button>
        <Button onClick={() => setIsAddReviewModalOpen(true)}>Add Review</Button>
      </div>
        ...
        <Modal>
          <ModalContent>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setIsAddReviewModalOpen(false)} className="text-gray-500 hover:text-gray-700">
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="mb-4">
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">Your Review</label>
              <TextArea ref={reviewTextRef} placeholder="Share your experience..." maxLength={500} />
            </div>
