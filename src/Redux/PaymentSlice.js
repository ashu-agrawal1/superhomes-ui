...
export const validatePayment = createAsyncThunk("payment/validatePayment", async (paymentData, { rejectWithValue }) => {
  try {
...
export const getAllReservations = createAsyncThunk("payment/getAllReservations", async (_, { rejectWithValue }) => {
  try {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(getAllReservations.fulfilled, (state, action) => {
        ...
        state.allReservations = Array.isArray(action.payload.data) ? action.payload.data : []
      })
    ...
    builder
      ...
      .addCase(getUserReservations.fulfilled, (state, action) => {
        state.loading = false
        state.reservations = Array.isArray(action.payload.data) ? action.payload.data : []
      })
      .addCase(getUserReservations.rejected, (state, action) => {
        state.loading = false
        state.error = action.payload?.message || "Failed to fetch user reservations"
        state.reservations = []
      });
  },
})
