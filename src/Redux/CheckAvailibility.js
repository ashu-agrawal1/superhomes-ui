...
const BASE_URL = baseurl + "reservation";
  ...
  async (propertyId, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `${BASE_URL}/getReservationByPropertyID/${propertyId}`
      );
...
export const addPropertyAvailability = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      console.error("API Error:", error);
      return rejectWithValue(
        error.response?.data?.error || "Failed to add availability"
      );
    }
  }
);
...
export const checkBookingConflicts = createAsyncThunk(
  ...
  async (
    { selectedStartDate, selectedEndDate, propertyId },
    { rejectWithValue }
  ) => {
    try {
      ...
      console.error("API Error:", error);
      return rejectWithValue(
        error.response?.data?.error || "Failed to check conflicts"
      );
    }
  }
);
